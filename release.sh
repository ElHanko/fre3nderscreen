...
RELEASES_DIR=./releases/fre3nderscreen
rm -rf $RELEASES_DIR
mkdir -p $RELEASES_DIR

ASSET_NAME=$1

"$CROSS_COMPILE"strip ./build/bin/fre3nderscreen
"$CROSS_COMPILE"strip ./build/bin/kd_graphic_mode
cp ./build/bin/fre3nderscreen $RELEASES_DIR/fre3nderscreen
...
echo "{\"version\": \"$FRE3NDERSCREEN_VERSION\", \"theme\": \"$GUPPY_THEME\", \"asset_name\": \"$ASSET_NAME.tar.gz\"}" > $RELEASES_DIR/.version
