...
const uint32_t FRE3NDER_COLOR_SIZE = 19;
const lv_palette_t FRE3NDER_COLORS[FRE3NDER_COLOR_SIZE] = {
  LV_PALETTE_RED,
  LV_PALETTE_PINK,
  LV_PALETTE_PURPLE,
...
json State::get_display_sensors() {
  ...
  if (display_sensors.empty()) {
    ...
    lv_palette_t color = FRE3NDER_COLORS[color_idx];
    for (auto &e: extruders) {
      spdlog::debug("default extruder {}", e);
      color = FRE3NDER_COLORS[color_idx % FRE3NDER_COLOR_SIZE];
      display_sensors[e] = {
	{ "id", e },
